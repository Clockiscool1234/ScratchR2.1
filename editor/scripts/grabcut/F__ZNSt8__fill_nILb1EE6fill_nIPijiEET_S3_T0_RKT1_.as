package grabcut
{
   import avm2.intrinsics.memory.li32;
   import avm2.intrinsics.memory.si32;
   
   public function F__ZNSt8__fill_nILb1EE6fill_nIPijiEET_S3_T0_RKT1_() : void
   {
      var _temp_1:* = this;
      var _loc3_:int = 0;
      var _loc1_:int = 0;
      var _loc2_:int = ESP;
      _loc3_ = _loc2_;
      _loc2_ -= 24;
      _loc1_ = li32(_loc3_);
      si32(_loc1_,_loc3_ - 4);
      _loc1_ = li32(_loc3_ + 4);
      si32(_loc1_,_loc3_ - 8);
      _loc1_ = li32(_loc3_ + 8);
      si32(_loc1_,_loc3_ - 12);
      _loc1_ = li32(_loc1_);
      si32(_loc1_,_loc3_ - 24);
      while(true)
      {
         _loc1_ = li32(_loc3_ - 8);
         if(_loc1_ == 0)
         {
            break;
         }
         si32(li32(_loc3_ - 24),li32(_loc3_ - 4));
         si32(int(li32(_loc3_ - 8) + -1),_loc3_ - 8);
         si32(int(li32(_loc3_ - 4) + 4),_loc3_ - 4);
      }
      var _temp_2:* = li32(_loc3_ - 4);
      si32(_temp_2,_loc3_ - 20);
      si32(_temp_2,_loc3_ - 16);
      eax = li32(_loc3_ - 16);
      ESP = _loc3_;
   }
}

import avm2.intrinsics.memory.*;
import grabcut.*;
import grabcut_2F_var_2F_folders_2F_6q_2F_k8f6r3f11mz7vklyw3c3_thc0000gn_2F_T_2F__2F_ccJkt6Tg_2E_o_3A_867FA3D3_2D_F96E_2D_404D_2D_9DBB_2D_0ACB3EFD0AC6.*;

