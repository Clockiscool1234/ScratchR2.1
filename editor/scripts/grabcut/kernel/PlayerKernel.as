package grabcut.kernel
{
   import flash.utils.ByteArray;
   import grabcut.CModule;
   import grabcut.ram;
   import grabcut.vfs.FileHandle;
   import grabcut.vfs.PathUtils;
   import grabcut.workerClass;
   import grabcut.yield;
   ...
   public class PlayerKernel implements IKernel
   {
      ...
      public function open(param1:int, param2:int, param3:int, param4:int) : int
      {
         ...
         if(!_loc12_ && _loc9_)
         {
            ...
            var _temp_5:* = PathUtils.getDirectory(_loc7_);
            var _loc10_:FileHandle = CModule.vfs.getFileHandleFromPath(_temp_5);
      ...
      public function mkdir(param1:int, param2:int, param3:int) : int
      {
         ...
         var _temp_3:* = PathUtils.getDirectory(_loc5_);
         var _loc6_:FileHandle = CModule.vfs.getFileHandleFromPath(_temp_3);
         if(!_loc6_.writeable)
         {
      ...
      public function dup(param1:int, param2:int) : int
      {
         ...
         if(CModule.vfs.isValidFileDescriptor(param1))
         {
            var _temp_1:* = CModule.vfs.getFileHandleFromFileDescriptor(param1);
            _loc3_ = CModule.vfs.openFile(_temp_1);
         }
         else
         {
            CModule.write32(param2,9);
      ...
      public function rename(param1:int, param2:int, param3:int) : int
      {
         ...
         var _temp_12:* = PathUtils.getDirectory(_loc6_);
         var _loc7_:FileHandle = CModule.vfs.getFileHandleFromPath(_temp_12);
         if(!_loc7_.writeable)
         {
            CModule.write32(param3,30);
            return -1;
      ...
      public function nanosleep(param1:int, param2:int, param3:int) : int
      {
         if(workerClass)
         {
            var _temp_1:* = uint(CModule.read32(param1));
            var _loc4_:uint = uint(CModule.read32(param1 + 4));
            var _loc6_:int = _temp_1 * 1000 + _loc4_ / 1000000;
            yield(_loc6_);
            if(param2)
            {
...
import grabcut.kernel.IKernel;
import grabcut.kernel.KernelConstants;
import grabcut.kernel.PlayerKernel;
