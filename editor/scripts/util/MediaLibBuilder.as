package util
{
   ...
   public class MediaLibBuilder
   {
      ...
      public static function checkJSONFile() : void
      {
         ...
         fileLoaded = function(param1:Event):void
         {
            ...
            try
            {
               ...
               _loc3_ = util.JSON.parse(_loc4_) as Array;
            }
            catch(e:*)
            {
            }
      ...
      private static function addCostume(param1:ScratchCostume, param2:String) : void
      {
         ...
         var _loc4_:String = "  {";
         _loc4_ += pair("name",param1.costumeName);
         _loc4_ += pair("md5",_loc3_);
         _loc4_ += pair("type",param2);
         _loc4_ += pair("tags",[]);
         _loc4_ += pair("info",costumeWidthHeight(param1),true);
         result += _loc4_;
      }
      ...
      private static function addSound(param1:ScratchSound) : void
      {
         ...
         var _loc4_:String = "  {";
         _loc4_ += pair("name",param1.soundName);
         _loc4_ += pair("md5",_loc2_);
         _loc4_ += pair("type","sound");
         _loc4_ += pair("tags",[]);
         _loc4_ += pair("info",[_loc3_],true);
         result += _loc4_;
      }
      ...
      private static function addSprite(param1:ScratchSprite, param2:String) : void
      {
         ...
         var _loc3_:String = "  {";
         _loc3_ += pair("name",param1.objName);
         _loc3_ += pair("md5",param2);
         _loc3_ += pair("type","sprite");
         _loc3_ += pair("tags",[]);
         _loc3_ += pair("info",[countScripts(param1),param1.costumes.length,param1.sounds.length],true);
         result += _loc3_;
      }
      ...
      {
         return "\"" + param1 + "\": " + util.JSON.stringify(param2) + (param3 ? "},\n" : ", ");
      }
   }
}
