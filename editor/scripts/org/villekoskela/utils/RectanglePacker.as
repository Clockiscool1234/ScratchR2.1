package org.villekoskela.utils
{
   ...
   public class RectanglePacker
   {
      ...
      public function RectanglePacker(param1:int, param2:int)
      {
         super();
      ...
      public function packRectangles(param1:Boolean = true) : int
      {
         ...
         while(this.mInsertList.length > 0)
         {
            ...
            _loc5_ = this.getFreeAreaIndex(_loc3_,_loc4_);
            if(_loc5_ >= 0)
            {
               _loc6_ = this.mFreeAreas[_loc5_];
               _loc7_ = this.allocateRectangle(_loc6_.x,_loc6_.y,_loc3_,_loc4_);
               _loc7_.id = _loc2_.id;
      ...
      private function generateNewFreeAreas(param1:IntegerRectangle, param2:Vector.<IntegerRectangle>, param3:Vector.<IntegerRectangle>) : void
      {
         ...
         while(_loc8_ >= 0)
         {
            ...
            {
               this.generateDividedAreas(param1,_loc9_,param3);
               _loc10_ = param2.pop();
      ...
      private function getFreeAreaIndex(param1:int, param2:int) : int
      {
         ...
         var _loc4_:int = -1;
         var _loc5_:int = int(this.mFreeAreas.length);
         var _loc6_:* = int(_loc5_ - 1);
         while(_loc6_ >= 0)
         {
            _loc7_ = this.mFreeAreas[_loc6_];
            if(_loc7_.x < _loc3_.x && param1 <= _loc7_.width && param2 <= _loc7_.height)
            {
               _loc4_ = _loc6_;
               if(param1 == _loc7_.width && _loc7_.width <= _loc7_.height && _loc7_.right < this.mWidth || param2 == _loc7_.height && _loc7_.height <= _loc7_.width)
               {
                  break;
               }
               _loc3_ = _loc7_;
      ...
      private function allocateRectangle(param1:int, param2:int, param3:int, param4:int) : IntegerRectangle
      {
         ...
         {
            _loc5_ = this.mRectangleStack.pop();
            _loc5_.x = param1;
            _loc5_.y = param2;
            _loc5_.width = param3;
      ...
      private function allocateSize(param1:int, param2:int, param3:int) : SortableSize
      {
         ...
         {
            _loc4_ = this.mSortableSizeStack.pop();
            _loc4_.width = param1;
